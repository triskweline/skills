...
Your final message is short, and it ends with the file. Anything you want to say about the run (unplaced hunks, a worker that had to be replaced, how many topics) goes first, in a sentence or two. Then, as the **last line of the message**, the tour's URL alone on its own line:

```
file:///tmp/vibe-tour.sLxCWm/vibe-tour.html
```

**Always a `file://` URL, never a bare path.** A URL is what the terminal turns into a link the human can click; a path is not. The assemble command prints the URL in exactly this form as the first thing on its result line, so copy it from there rather than building it yourself. Nothing after it. The human is scanning for the link, and a path buried in a paragraph or followed by more prose is the thing they will not find.
